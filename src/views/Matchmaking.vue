...
<template>
  <BasePage title="Matchmaking" class="max-md:w-full max-md:p-2">
    <header class="w-full flex justify-between flex-wrap gap-3 max-w-full max-md:px-2">
      <h2 class="text-left">
        Sélectionnez vos jeux préférés et indiquez votre niveau pour chaque jeu. Plus vous ajoutez
        de jeux, plus vos chances de trouver des correspondances adéquates augmentent.
      </h2>
      <!-- Jeux sélectionnés -->
          ...
          <DialogScrollContent class="flex flex-col gap-6">
            <form @submit.prevent="handleSubmit">
              ...
              <FormField name="gameId">
                <FormItem>
                  <FormLabel :class="{ 'text-red-500': gameError }"
                    >Veuillez sélectionner un jeu.</FormLabel
                  >
                  <FormControl>
              ...
              <FormField name="levelId">
                <FormItem class="flex flex-col">
                  <FormLabel :class="{ 'text-red-500': levelError }"
                    >Veuillez sélectionner votre niveau.</FormLabel
                  >
                  <FormControl class="mt-2">
                    <SkillsCombobox
                      v-model="selectedLevel"
                      @update:modelValue="handleSelectLevel"
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
    ...
    <div class="border border-custom-white rounded-lg p-4">
      <p v-if="selectedGames.length === 0" class="text-gray-400 text-center">
        Sélectionnez des jeux pour trouver des joueurs correspondant à votre profil
      </p>

      <!-- Jeux sélectionnés mais aucune correspondance -->
      <p v-else-if="matchResults.length === 0" class="text-center">
        <span class="font-medium">Aucun joueur trouvé pour le moment</span><br />
        <span class="text-gray-400 text-sm mt-2"
          >Revenez plus tard ou ajoutez d'autres jeux pour augmenter vos chances de trouver des
      ...
      <template v-else>
        <p class="mb-4">
          <span class="font-medium"
            >{{ matchResults.length }} joueur{{ matchResults.length > 1 ? 's' : '' }} trouvé{{
              matchResults.length > 1 ? 's' : ''
            }}</span
          >
          <span class="text-gray-400"> correspondant à vos critères de recherche</span>
        </p>
        <div class="results-container grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          <router-link
          ...
          >
            ...
            <p class="text-xs text-opacity-50">Correspondance</p>
